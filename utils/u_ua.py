...
def random_int(mix_num: int, max_num: int) -> int:
    """从两个数之间取一个随机数"""
...
def random_one() -> str:
    """
      ...
      :return:
      """
    r_user_agent_str = get_r_user_agent().pc()
    if r_user_agent_str:
        # 如果第三方随机user_agent有值，则优先返回第三方生成的，原先本地固定的user_agent数量有限，可能更容易被风控
        return r_user_agent_str

    v = random.choice(user_agent_list)
    v = f"{v[:-2]}{random_int(0, 100)}"
    return str(v)
